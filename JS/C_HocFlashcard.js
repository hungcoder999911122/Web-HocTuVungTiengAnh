document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleAssessment(status) {
        ...
        // 👉 Gửi dữ liệu xuống Database ngay khi bấm nút
        fetch('C_HocFlashcard.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: new URLSearchParams({
                action: 'luu_tien_do',
                vocabulary_id: cardId,
                status: status // 'da_nho' hoặc 'chua_nho'
            })
        })
        .then(res => res.json())
        .then(data => console.log("Đã lưu Database:", data))
        ...
        if (currentIndex < cards.length - 1) {
            ...
            setTimeout(() => {
                ...
                window.location.href = "C_Dashboard_user.php"; // Học xong tự về Dashboard
            }, 100);
        }
    }
